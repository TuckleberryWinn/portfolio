...
router.get("/", (ctx) => {
    ...
    <html>
      <head><title>Hello API!</title><head>
      <body>
...
router.post("/submit", async (ctx) => {
    const parsed = await ctx.request.body.json();
    //Validating form data
    if (!parsed.id) {
        throw new Error("Destination is required.");
    }
    ...
    const encodedNewData = JSON.stringify(projectData);
    await fs.writeFile(dataFilePath, encodedNewData);
    ctx.response.body = encodedNewData;
});

const app = new Application();

app.use(async (ctx, next) => {
    ctx.response.headers.set("goats-were-here", true);
    try {
        await next();
    } catch (err) {
...
app.use(async (ctx, next) => {
    console.log("What is cxt.request.url?:", ctx.request.url);
    if (ctx.request.url.pathname === "/") {
        ctx.response.headers.set("ibuprofen-were-here", true);
    }
    await next();
});
...
const oakPort = 8085;
app.listen({ port: oakPort });
console.log(`Oak server listening on http://localhost:${oakPort}`);
